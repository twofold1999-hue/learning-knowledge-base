...
describe('默认 AI 入口到本地代理', () => {
  it('非空笔记点击整理会调用 AIService.summarizeNote 并发出同源 POST', async () => {
    ...
    await act(async () => { root?.render(<AINoteOrganizer getCurrentContent={() => "# 非空笔记"} noteId="proxy_summary_note" onApply={vi.fn()} />) })
  ...
  it('非空笔记点击知识分析会调用 AIService.extractKnowledgeCandidates 并发出新的同源 POST', async () => {
    ...
    container = document.createElement('div'); document.body.append(container); root = createRoot(container)
    await act(async () => { root?.render(<AIKnowledgeAnalyzer getCurrentContent={() => "# CPU"} noteId="proxy_candidate_note" />) })
  ...
  it('空正文会在组件层明确拒绝，不会伪装成网络失败或发送请求', async () => {
    ...
    const { default: AINoteOrganizer } = await import('./AINoteOrganizer')
    container = document.createElement('div'); document.body.append(container); root = createRoot(container)
    await act(async () => { root?.render(<AINoteOrganizer getCurrentContent={() => "   "} onApply={vi.fn()} />) })
