...
控制中心显示产品名、版本、数据目录准备状态和上次可能的异常退出提示。当前可用操作只有：

- 启动并进入知识库；
- 退出程序。

“配置 AI”、“导入浏览器版 Backup”、“打开备份目录”和“查看日志”均为禁用的后续功能，不绑定替代行为。
...
9. 确认关闭应用后没有本项目的后台服务、CMD 或 PowerShell 窗口。

## 本阶段未实现

本阶段不实现 Credential Manager/SecretStore、AI 传输迁移、Backup 文件迁移或恢复、打开备份目录、日志查看和轮转、删除全部数据、安装器/WebView2 策略、代码签名、系统托盘、自动更新、多窗口或 Node sidecar，也不改变 Dexie v11 或 Backup v5。